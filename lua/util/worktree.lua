...
-- Helper to check if Neo-tree sidebar is currently open in the active tab
...
function M.switch_worktree(path, opts_switch)
  ...
  -- Detect if Neo-tree is open before shutting down the session
  local neotree_was_open = check_neotree_open()

  -- 1. Store active file path to preserve focus
  vim.g.SessionLastFile = vim.fn.expand("%:p")

  -- 2. Close Neo-tree sidebar and clear arglist to prevent session pollution
  ...
  -- 9. Restore Neo-tree sidebar if it was open previously (unless skipped)
